  ...
  async handleMessage(
    ...
    try {
      ...
      const populatedMessage = await (message as any).populate([
        { path: 'sender_id', select: 'username display_name avatar_url rank equipped_accent' },
        { path: 'reply_to', select: 'text sender_id', populate: { path: 'sender_id', select: 'username display_name avatar_url rank equipped_accent _id' } },
      ]);
